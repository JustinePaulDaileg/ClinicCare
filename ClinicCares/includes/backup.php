...
class BackupManager
{
            ...
            if (class_exists('ZipArchive')) {
                ...
                if ($zip->open($zippath, ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
                    ...
                    if (file_exists($zippath) && filesize($zippath) > 0) {
                        ...
                        @unlink($zippath);      // broken zip, remove it
                        $finalPath = $filepath; // fallback: keep .sql
                        $finalFile = $filename;
                    }
                } else {
                    $finalPath = $filepath;     // ZipArchive failed, keep .sql
                    $finalFile = $filename;
                }
            } else {
                $finalPath = $filepath;
    ...
    public function restoreBackup(string $filename, int $adminId): array
    {
        ...
        try {
            ...
            if (substr($filename, -4) === '.zip' && class_exists('ZipArchive')) {
                $zip = new ZipArchive();
    ...
    public function downloadBackup(string $filename): void
    {
        ...
        $mime = substr($filename, -4) === '.zip' ? 'application/zip' : 'application/octet-stream';
